...
const byte loadpointId = 1; // ab evcc 0.111.0 Nr. 1-255 <--- bitte mit Browser in API nachschauen
  ...
  if (millis() - lastTime >= 1000) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (httpCode == HTTP_CODE_OK) {
        ...
        int doclen = payload.length();
        //Serial.println(doclen);
        //Serial.println(ESP.getFreeHeap());
        DynamicJsonDocument doc(doclen + 200); //Größe der JSON Datei je nach größe der API z.B.: 2048 oder 4096 Byte
        deserializeJson(doc, payload);
        //Serial.println(ESP.getFreeHeap());
        //Serial.println("Extract the required data");  //Debug
      
        // Extract the required data
          int batteryPower = doc["result"]["batteryPower"];
          //int invertedBatteryPower = -1 * batteryPower;
          int batterySoc = doc["result"]["batterySoc"];
          int gridPower = doc["result"]["gridPower"];
          int homePower = doc["result"]["homePower"];
          int pvPower = doc["result"]["pvPower"];
          int chargePower = doc["result"]["loadpoints"][loadpointId]["chargePower"];
          bool connected = doc["result"]["loadpoints"][loadpointId]["connected"];
          //bool connected = true;
          bool charging = doc["result"]["loadpoints"][loadpointId]["charging"];
          //charging = true;
          int vehicleSoc = doc["result"]["loadpoints"][loadpointId]["vehicleSoc"];
          float chargeRemainingDurationLongFloat = doc["result"]["loadpoints"][loadpointId]["chargeRemainingDuration"];
          String mode = doc["result"]["loadpoints"][loadpointId]["mode"];
          //chargeRemainingDurationLongStr = "4409822173901";
          //int vehicleSoc = 80;
          //String vehicleTitle = "Gastfahrzeug";
          String vehicleTitle = doc["result"]["loadpoints"][loadpointId]["vehicleTitle"];
        //auf rechtbündig und Links Formatieren
          ...
          char vehicleTitleStr[7];        //6 = Anzahl der Stellen + 1 (Fahrzeug Name wird nch 4 Zeichen abgeschitten)
          sprintf(vehicleTitleStr, "%-6.6s", vehicleTitle.substring(0, 5) + ":");  //vehicleTitleStr = Augabe, %-6.6s = Anzahl freier Stellen, vehicleTitle = Datenquelle
          ...
          //long chargeRemainingDurationLong = chargeRemainingDurationLongStr.substring(0, 8).to(); //String 4409822173901 in int 4409 umwandeln
          char chargeRemainingDurationStr[20]; //20 = Anzahl der Stellen
          long chargeRemainingDurationLong = chargeRemainingDurationLongFloat / 1000000000;
          if (chargeRemainingDurationLong >= 0 && charging) {
            int chargeRemainingDurationSeconds = (chargeRemainingDurationLong) % 60;
            int chargeRemainingDurationMinutes = (chargeRemainingDurationLong / 60) % 60;
            int chargeRemainingDurationHours = (chargeRemainingDurationLong / 3600) % 24;
            sprintf(chargeRemainingDurationStr, "%02d:%02d:%02d h", chargeRemainingDurationHours, chargeRemainingDurationMinutes, chargeRemainingDurationSeconds);
            //Serial.printf("Restzeit: %02d:%02d:%02d\n", chargeRemainingDurationHours, chargeRemainingDurationMinutes, chargeRemainingDurationSeconds);
          }
        ...
        // Display the extracted data on OLED display
          ...
          String texts[] = {
            ...
            "Netzbezug: " + String(gridPowerStr) + " W",
            String(connected ? String(vehicleTitleStr) + String(vehicleSocStr) + "% " + String(chargePowerStr) : "kein Auto " + String(modeStr)), //+ ":"
